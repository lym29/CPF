...
def batch_mesh_contains_points(
    ...
    dets = torch.bmm(v0v1.view(batch_points_size, 1, 3), pvec.view(batch_points_size, 3,
    ...
    hand_verts_repeated = (ray_origins.view(batch_size, point_nb, 1,
                                            3).repeat(1, 1, triangle_nb, 1).view(ray_origins.shape[0],
...
def penetration_loss_hand_in_obj(hand_verts, obj_verts, obj_faces, mode="max"):
    ...
    exterior = batch_mesh_contains_points(hand_verts.detach(),
                                          obj_triangles.detach())  # exterior computation transfers no gradients
...
def solid_intersection_volume(hand_verts, hand_faces, obj_vox_points, obj_tsl, obj_rot, obj_vox_el_vol):
    ...
    if obj_rot.shape == (3, 3):
        ...
        obj_rotmat = aa_to_rotmat(obj_rot)
...
def region_disjointness_metric(hand_verts_pred, obj_verts_pred, hand_region_assignment):
    ...
    for region_id in all_regions:
        ...
        # cross distance
        dist_mat = np.abs(np_pairwise_dist(hand_verts_pred_region, obj_verts_pred))  # make sure positive
