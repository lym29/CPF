...
def process_contact_info(
    ...
    vertex_contact = np.array(vertex_contact, dtype=int)
    if pad_vertex and pad_anchor:
        ...
        hand_region = np.array(hand_region, dtype=int)
        ...
        for item in contact_info:
            if item["contact"] == 1:
                ...
                item_anchor_padding_mask = np.zeros((anchor_padding_len,), dtype=int)
        ...
        anchor_id = np.array(anchor_id, dtype=int)
        anchor_dist = np.array(anchor_dist)
        anchor_elasti = np.array(anchor_elasti)
        anchor_padding_mask = np.array(anchor_padding_mask, dtype=int)
