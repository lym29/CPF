...
import numpy as np
import torch
from manotorch.utils.quatutils import _quaternion_norm_squared, _quaternion_inv, _quaternion_mul

from lib.utils.collision import th_pairwise_dist
...
class FieldLoss:
    ...
    def repulsion_loss(
        ...
        thresholded_value = constant * torch.pow(torch.exp(torch.clamp(-inner_product, -threshold, threshold)),
                                                 2)  # TENSOR[NCC, ]
    ...
    def full_repulsion_loss(
        ...
        # pairwise dist
        dist_mat = th_pairwise_dist(pred_full_obj_verts, pred_hand_verts)
...
class HandLoss:
    ...
    def pose_quat_norm_loss(var_pose):
        ...
        quat_norm_sq = _quaternion_norm_squared(reshaped_var_pose)  # TENSOR[16, ]
    ...
    def pose_reg_loss(var_pose_normed, var_pose_init):
        ...
        inv_var_pose_init = _quaternion_inv(var_pose_init)
        combined_pose = _quaternion_mul(var_pose_normed, inv_var_pose_init)
    ...
    def joint_b_axis_loss(b_axis, axis):
        ...
        res = (torch.mean(torch.pow(restrict_cos, 2), dim=0) + torch.mean(torch.pow(soft_loss, 2), dim=0) +
    ...
    def joint_l_limit_loss(l_axis, axis):
        ...
        res = (torch.mean(torch.pow(restrict_cos - 1, 2), dim=0) + torch.mean(torch.pow(soft_loss, 2), dim=0) +
               0.01 * torch.mean(torch.pow(thumb_soft_loss, 2), dim=0))
        return res
