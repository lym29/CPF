...
class VizContext:
    ...
    @import_open3d
    def update_by_arrow(self, geo_key, rotate, translate, colors=None, update=True, o3d=None):
        if self.geometry_to_viz.get(geo_key) and not update:
            return

        if isinstance(rotate, torch.Tensor):
            rotate = rotate.detach().cpu().numpy()

        if isinstance(translate, torch.Tensor):
            translate = translate.detach().cpu().numpy()

        colors = self.paint_color_on(translate, colors).reshape(*translate.shape)
        ...
        if self.geometry_to_viz.get(geo_key) is None:  # create content
            ...
            arrow.paint_uniform_color(colors)
            arrow.compute_vertex_normals()
            arrow = arrow.rotate(rotate, center=(0, 0, 0))
            arrow = arrow.translate(translate)

            self.vis.add_geometry(arrow)
            self.geometry_to_viz[geo_key] = arrow

        else:
            arrow = self.geometry_to_viz[geo_key].rotate(rotate, center=(0, 0, 0))
            arrow = arrow.translate(translate)
            self.geometry_to_viz[geo_key] = arrow
